...
class NewWorkspaceOverlay(QWidget):
    submitted = pyqtSignal(str, str)  # name, hex_color
    ...
    def _build(self):
        ...
        self.input.setPlaceholderText("Company or project name…")
        ...
        crow.setSpacing(8)
        clbl = QLabel("Color:")
        clbl.setStyleSheet(f"color: {C['text2']}; font-size: 11px; background: transparent; border: none;")
        crow.addWidget(clbl)
        self._swatch = QLabel()
        self._swatch.setFixedSize(20, 20)
        crow.addWidget(self._swatch)
        self._color_inp = QLineEdit()
        self._color_inp.setFixedHeight(28)
        self._color_inp.setStyleSheet(f"""
            QLineEdit {{ background: {C['surface2']}; border: 1px solid {C['border2']};
                border-radius: 5px; color: {C['text']}; font-size: 11px; padding: 0 8px;
                font-family: monospace; }}
            QLineEdit:focus {{ border-color: {C['accent']}; }}
        """)
        self._color_inp.textChanged.connect(self._upd_swatch)
        crow.addWidget(self._color_inp, 1)
        cyc = QPushButton("↻")
        cyc.setFixedSize(28, 28)
        cyc.setStyleSheet(f"""
        ...
        """)
        cyc.clicked.connect(self._cycle)
        crow.addWidget(cyc)
        cl.addLayout(crow)
        ...
        for t in ["Enter to create", "Esc to cancel"]:
            l = QLabel(t)
            l.setStyleSheet(f"color: {C['text3']}; font-size: 10px; background: transparent; border: none;")
            hrow.addWidget(l)
            hrow.addStretch()
        cl.addLayout(hrow)
    ...
    def open(self):
        ...
        self._color_inp.setText(color)
        self._upd_swatch(color)
        self._reposition()
        self.show()
        self.raise_()
        self.input.setFocus()
    ...
    def _submit(self):
        ...
        NewWorkspaceOverlay._cidx += 1
        self.submitted.emit(name, color)
        self.hide()
    ...
    def resizeEvent(self, e):
        super().resizeEvent(e); self._reposition()

    def showEvent(self, e):
        super().showEvent(e); self._reposition()


# ── Workspace dropdown (centered topbar) ───────────────────────────────────────
...
class WorkspaceDropdown(QWidget):
    ...
    def _show_menu(self):
        ...
        for i, name in enumerate(workspaces):
            color = get_workspace_color(name)
...
class TimeEntryOverlay(QWidget):
    ...
    def _build(self):
        ...
        self._day_label.setStyleSheet(
            f"color: {C['text2']}; font-size: 11px; letter-spacing: 0.5px; background: transparent; border: none;")
        ...
        self.input.setStyleSheet(f"""
            QLineEdit {{
                ...
                font-family: 'JetBrains Mono', 'Fira Code', monospace; padding: 0 16px; letter-spacing: 2px;
            }}
            QLineEdit:focus {{ border-color: {C['accent']}; background: {C['surface2']}; color: {C['text']}; }}
        """)
        self.input.returnPressed.connect(self._submit)
        ...
        for t, s in [("Enter to save", f"color: {C['text3']}; font-size: 10px;"),
            ...
            l = QLabel(t)
            l.setStyleSheet(f"{s} background: transparent; border: none;")
...
class EntryWidget(QWidget):
    ...
    def _build(self):
        ...
        if self._ws_color:
            ...
            dot.setStyleSheet(f"color: {self._ws_color}; font-size: 8px;")
...
class DayColumn(QWidget):
    ...
    def __init__(self, day: date, entries: list, C: dict, is_today: bool,
        ...
        self._entry_colors: list[str | None] = []
        for e in entries:
            if isinstance(e, tuple):
                self._entries.append(e[0]); self._entry_colors.append(e[1])
            else:
                self._entries.append(e); self._entry_colors.append(None)
    ...
    def _build(self):
        ...
        scroll = QScrollArea(); scroll.setWidget(self._entries_container)
    ...
    def _rebuild_entries(self):
        for w in self._entry_widgets:
            self._entries_layout.removeWidget(w); w.deleteLater()
    ...
    def _delete_entry(self, index: int):
        for i, ew in enumerate(self._entry_widgets):
            if ew.index == index:
                self._entries.pop(i)
                if i < len(self._entry_colors):
                    self._entry_colors.pop(i)
                break
    ...
    def remove_entry_at(self, idx: int):
        ...
        if 0 <= idx < len(self._entry_colors):
            self._entry_colors.pop(idx)
        self._rebuild_entries(); self._refresh_total(); self.data_changed.emit()
    ...
    def set_hovered(self, on: bool):
        ...
        if on:
            pal = self.palette()
            pal.setColor(self.backgroundRole(), QColor(col))
            self.setPalette(pal)
        else:
            self.setAutoFillBackground(False)

    def enterEvent(self, e):
        self._hovered = True; self.set_hovered(True)
        self.hovered_day.emit(self.day, True)

    def leaveEvent(self, e):
        self._hovered = False; self.set_hovered(False)
    ...
    def dropEvent(self, e):
        if self._read_only or not e.mimeData().hasFormat(DRAG_MIME):
            return
        ...
        for col in self.window()._columns:
            if col.day.isoformat() == src_day_str:
                col.remove_entry_at(src_idx); break
        self.add_entry(mins)
...
class MarqueeLabel(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._text = ""; self._offset = 0.0; self._speed = 0.4
        self._text_w = 0; self._gap = 80
    ...
    def _tick(self):
        if not self._text or not self._text_w:
            return
    ...
    def paintEvent(self, event):
        ...
        fm = self.fontMetrics(); y = fm.ascent() + (self.height() - fm.height()) // 2
        total = self._text_w + self._gap; x = -int(self._offset)
        while x < self.width():
            painter.drawText(x, y, self._text); x += total
        painter.end()
...
class SheetTargetDialog(QDialog):
    ...
    def __init__(self, sheet: dict, C: dict, parent=None):
        ...
        note = QLabel("Overrides workspace defaults for this sheet only.")
        note.setStyleSheet(f"color: {C['text3']}; font-size: 10px;"); note.setWordWrap(True)
        ...
        save.clicked.connect(self._save); br.addWidget(save)
        layout.addLayout(br)
...
class WorkspaceSettingsDialog(QDialog):
    ...
    def __init__(self, name: str, cfg: dict, C: dict, parent=None):
        ...
        title = QLabel(f"WORKSPACE — {name.upper()}")
        title.setStyleSheet(f"color: {C['text']}; font-size: 10px; letter-spacing: 2px;")
        layout.addWidget(title)
        ...
        cl_lbl = QLabel("Workspace Color"); cl_lbl.setStyleSheet(f"color: {C['text2']}; font-size: 11px;"); cl_lbl.setFixedWidth(160)
        cr.addWidget(cl_lbl)
        ...
        self._color_inp.textChanged.connect(self._upd_swatch)
        self._upd_swatch(cfg.get("color", "#4f7cff"))
        ...
        self.pay_type = QComboBox(); self.pay_type.addItems(["Hourly", "Salary"])
        self.pay_type.setCurrentText(cfg.get("pay_type", "hourly").capitalize())
        layout.addWidget(mk("Default Pay Type", self.pay_type))

        self.hourly = QDoubleSpinBox(); self.hourly.setRange(0, 10000); self.hourly.setDecimals(2)
        self.hourly.setPrefix("$ "); self.hourly.setValue(cfg.get("hourly_rate", 0.0))
        self.hourly.setButtonSymbols(QDoubleSpinBox.ButtonSymbols.NoButtons)
        layout.addWidget(mk("Default Hourly Rate", self.hourly))

        self.salary = QDoubleSpinBox(); self.salary.setRange(0, 10_000_000); self.salary.setDecimals(2)
        self.salary.setPrefix("$ "); self.salary.setSingleStep(1000)
        self.salary.setValue(cfg.get("annual_salary", 0.0))
        self.salary.setButtonSymbols(QDoubleSpinBox.ButtonSymbols.NoButtons)
        layout.addWidget(mk("Default Annual Salary", self.salary))
        ...
        self.target.setButtonSymbols(QDoubleSpinBox.ButtonSymbols.NoButtons)
        layout.addWidget(mk("Default Target Hours", self.target))
        ...
        br.addWidget(self._del_btn); br.addStretch()
        cancel = QPushButton("Cancel"); cancel.setFixedWidth(70); cancel.clicked.connect(self.reject); br.addWidget(cancel)
        save = QPushButton("Save"); save.setFixedWidth(70)
        save.setStyleSheet(f"QPushButton {{ background: {C['accent']}; border: none; color: white; border-radius: 5px; font-size: 11px; }} QPushButton:hover {{ background: {C['accent2']}; }}")
        save.clicked.connect(self._save); br.addWidget(save)
...
class TimesheetApp(QMainWindow):
    def __init__(self):
        ...
        # Ensure at least one real workspace exists
        ...
        if startup and startup in available:
            self._active_workspace = startup
        elif last and last in available:
            self._active_workspace = last
        else:
            self._active_workspace = available[1] if len(available) > 1 else available[0]
    ...
    def _build_all_week_entries(self, week_start: date) -> dict:
        ...
        for name, (data, color) in self._all_ws_data().items():
            for key, sheet in data.items():
                try:
                    s = date.fromisoformat(sheet["start_date"])
                    e = date.fromisoformat(sheet["end_date"])
                except Exception:
                    continue
                if s > week_end or e < week_start:
                    continue
                for ds, entries in sheet.get("days", {}).items():
                    if ds in days:
                        for m in entries:
                            if m:
                                days[ds].append((m, color))
        return days
    ...
    def _mk_topbar(self) -> QWidget:
        ...
        layout.addWidget(self._ws_dropdown)
        layout.addStretch(1)
    ...
    def _mk_marquee_bar(self) -> QWidget:
        C = self._C
        bar = QWidget(); bar.setFixedHeight(26); bar.setStyleSheet(f"background: {C['bg']};")
    ...
    def _mk_nav(self) -> QWidget:
        C = self._C
        nav = QWidget(); nav.setFixedHeight(42); nav.setStyleSheet(f"background: {C['surface']};")
    ...
    def _mk_progress(self) -> QWidget:
        C = self._C
        w = QWidget(); w.setFixedHeight(52); w.setStyleSheet(f"background: {C['surface']};")
        ...
        bottom.addWidget(self.target_lbl); layout.addLayout(bottom)
        return w
    ...
    def _load_sheet_for_date(self, d: date):
        if self._active_workspace == ALL_WS:
            self._load_all_view_for_date(d)
            return
    ...
    def _load_all_view_for_date(self, d: date):
        ...
        offset = (d.weekday() - wsd) % 7
        ws = d - timedelta(days=offset)
        self._all_week_start = ws
        self._current_key = f"all_{ws.isoformat()}"
        self._current_sheet = {
        ...
        try:
            ...
            if is_all:
                ...
                self.sheet_lbl.setText(
                    f"All workspaces  ·  {start.strftime('%b %-d')} – {end.strftime('%b %-d, %Y')}  ·  {legend}"
                )
            else:
                self.sheet_lbl.setTextFormat(Qt.TextFormat.PlainText)
                self.sheet_lbl.setText(
    ...
    def keyPressEvent(self, event):
        ...
        if text and (text.isdigit() or text == ":") and not self._any_input_focused():
            if self._active_workspace != ALL_WS:
                self._open_entry_overlay(self._hovered_day or date.today(), text)
                return
    ...
    def _refresh_deadline(self):
        if self._active_workspace == ALL_WS:
            self._deadline_lbl.setText(""); return
        ...
        if is_all:
            ...
            for name, (data, color) in self._all_ws_data().items():
                ...
                for key, sheet in data.items():
                    for ds, entries in sheet.get("days", {}).items():
                        if ds in week_dates:
                            mins = sum(e for e in entries if e)
                            g, n = D.calc_earnings(mins, sheet, ws_s); tg += g; tn += n
            if tg == 0:
                self.earnings_lbl.setText("")
            else:
    ...
    def _refresh_marquee(self):
        ...
        if is_all:
            ...
            for name, (data, color) in self._all_ws_data().items():
                ws_mins = sum(
                    m for key, sheet in data.items()
                    for ds, entries in sheet.get("days", {}).items()
                    if ds in week_dates for m in entries if m
                )
                if ws_mins:
                    parts.append(f"● {name}: {D.format_hm_pretty(ws_mins)}")
    ...
    def _prev_sheet(self):
        if self._active_workspace == ALL_WS:
            self._load_all_view_for_date(self._all_week_start - timedelta(days=1))
            return
        self._do_save()
        r = D.get_sheet_by_offset(self._data, self._current_key, -1)
        if r:
            self._current_key, self._current_sheet = r; self._render_sheet()

    def _next_sheet(self):
        if self._active_workspace == ALL_WS:
            self._load_all_view_for_date(self._all_week_start + timedelta(days=7))
            return
        self._do_save()
        r = D.get_sheet_by_offset(self._data, self._current_key, +1)
        if r:
            ...
            try:
                next_day = date.fromisoformat(self._current_sheet["end_date"]) + timedelta(days=1)
            except Exception:
    ...
    def _show_status_toast(self):
        total = sum(col.get_total_minutes() for col in self._columns)
        if self._active_workspace == ALL_WS:
            self._toast.show_toast(f"All workspaces: {D.format_hm_pretty(total)} this week", "info"); return
        target_mins = int(self._current_sheet.get("target_hours", 40.0) * 60)
        remaining   = max(0, target_mins - total)
    ...
    def _on_sheet_override(self, tgt: float, hourly: float, pay_type: str, salary: float):
        self._current_sheet.update({"target_hours": tgt, "hourly_rate": hourly,
    ...
    def _open_new_workspace_overlay(self):
        overlay = self._new_ws_overlay
        overlay.setGeometry(self.centralWidget().rect()); overlay.open()

    def _on_new_workspace_created(self, name: str, color: str):
        if name == ALL_WS:
            self._toast.show_toast(f"'{ALL_WS}' is reserved", "error"); return
        if not create_workspace(name, color):
            self._toast.show_toast(f"'{name}' already exists", "error"); return
        self._toast.show_toast(f"Workspace '{name}' created", "success")
        self._switch_workspace(name)

    def _open_workspace_settings(self):
        if self._active_workspace == ALL_WS:
            self._toast.show_toast("No settings for All view", "info"); return
    ...
    def _on_workspace_settings_saved(self, result: dict):
        if result.get("__delete__"):
            ...
            self._switch_workspace(available[0] if available else ALL_WS)
            self._toast.show_toast(f"Workspace '{old}' deleted", "info"); return
    ...
    def _switch_workspace(self, name: str):
        ...
        self._settings["last_workspace"] = name
        D.save_settings(self._settings)
    ...
    def _show_export_menu(self):
        C = self._C
        dlg = QDialog(self); dlg.setWindowTitle("Export"); dlg.setFixedWidth(260)
        ...
        hdr = QLabel("EXPORT"); hdr.setStyleSheet(f"color:{C['text']};font-size:11px;letter-spacing:2px;")
        layout.addWidget(hdr)

        self._export_notes = False
        cb = QCheckBox("Include notes in PDF"); cb.setStyleSheet(f"color: {C['text2']}; font-size: 11px;")
        ...
        for lbl, fn in exports:
            b = QPushButton(lbl); b.clicked.connect(lambda _, f=fn: (dlg.close(), f()))
            layout.addWidget(b)
        dlg.exec()
    ...
    def _export_csv(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Export CSV",
            str(Path.home() / f"timesheet_{self._active_workspace}_sheet{self._current_sheet.get('sheet_num','X')}.csv"),
            "CSV Files (*.csv)"
        )
        if not path: return
    ...
    def _export_all_csv(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Export All", str(Path.home() / f"{self._active_workspace}_all.csv"), "CSV Files (*.csv)"
        )
        if not path: return
        with open(path, "w", newline="") as f:
            w = csv.writer(f)
    ...
    def _export_all_workspaces_csv(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Export All Workspaces",
            str(Path.home() / "timesheet_all_workspaces.csv"), "CSV Files (*.csv)"
        )
        if not path: return
        with open(path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["Workspace","Sheet","Date","Day","Entry","Minutes","Hours"])
            for name, (data, color) in self._all_ws_data().items():
                for key, sheet in sorted(data.items(), key=lambda x: x[1].get("sheet_num", 0)):
                    for ds, entries in sorted(sheet.get("days", {}).items()):
                        ...
                        for i, mins in enumerate(entries):
                            if mins:
                                w.writerow([name,key,ds,d.strftime("%A"),i+1,mins,round(mins/60,2)])
        self._toast.show_toast(f"Exported: {Path(path).name}", "success")
    ...
    def _export_pdf(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export PDF",
            str(Path.home() / f"timesheet_{self._active_workspace}_sheet{self._current_sheet.get('sheet_num','X')}.pdf"),
            "PDF Files (*.pdf)"
        )
        if not path: return
        sheet  = self._current_sheet
        ...
        try:
            ...
            story.append(Paragraph(
                f"Timesheet — {self._active_workspace}  ·  Sheet {sheet.get('sheet_num','?')}  ·  "
    ...
    def resizeEvent(self, e):
        ...
        if central:
            ...
            if hasattr(self, "_new_ws_overlay"):
                self._new_ws_overlay.setGeometry(central.rect())
