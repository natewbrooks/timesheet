...
def field_lbl(text, C):
    l = QLabel(text); l.setStyleSheet(f"color: {C['text2']}; font-size: 11px;"); l.setFixedWidth(170)
    return l


def make_row(lbl_widget, field_widget):
    w = QWidget(); w.setStyleSheet("background: transparent;")
    hl = QHBoxLayout(w); hl.setContentsMargins(0, 2, 0, 2); hl.setSpacing(12)
    hl.addWidget(lbl_widget); hl.addWidget(field_widget, 1); return w


class ColorRow(QWidget):
    def __init__(self, key: str, label: str, value: str, C: dict, parent=None):
        super().__init__(parent)
        self.key = key; self.setStyleSheet("background: transparent;")
        hl = QHBoxLayout(self); hl.setContentsMargins(0, 2, 0, 2); hl.setSpacing(8)
        lbl = QLabel(label); lbl.setStyleSheet(f"color: {C['text2']}; font-size: 11px;"); lbl.setFixedWidth(170)
        hl.addWidget(lbl)
        self.swatch = QLabel(); self.swatch.setFixedSize(20, 20)
        self.swatch.setStyleSheet(f"background: {value}; border-radius: 4px; border: 1px solid {C['border2']};")
        hl.addWidget(self.swatch)
        self.inp = QLineEdit(value); self.inp.setPlaceholderText("#rrggbb")
        self.inp.textChanged.connect(self._update_swatch); hl.addWidget(self.inp, 1)
...
class PanelPay(QWidget):
    def __init__(self, settings: dict, C: dict, parent=None):
        ...
        layout = QVBoxLayout(self); layout.setContentsMargins(24, 16, 24, 16)
        layout.setSpacing(8); layout.setAlignment(Qt.AlignmentFlag.AlignTop)
        ...
        self.pay_period = QComboBox(); self.pay_period.addItems(PAY_PERIODS)
        ...
        layout.addWidget(sep(C)); layout.addWidget(section_lbl("COMPENSATION", C))

        self.hourly_rate = QDoubleSpinBox(); self.hourly_rate.setRange(0, 10000)
        self.hourly_rate.setDecimals(2); self.hourly_rate.setPrefix("$ ")
        ...
        self.annual_salary = QDoubleSpinBox(); self.annual_salary.setRange(0, 10_000_000)
        self.annual_salary.setDecimals(2); self.annual_salary.setPrefix("$ ")
        self.annual_salary.setSingleStep(1000); self.annual_salary.setValue(settings.get("annual_salary", 0))
        ...
        layout.addWidget(sep(C)); layout.addWidget(section_lbl("TAX — US STATE", C))

        self.us_state = QComboBox()
        states = sorted(STATE_TAX_RATES.keys()); self.us_state.addItems(states)
        cur = settings.get("us_state", "CA")
        if cur in states: self.us_state.setCurrentText(cur)
        layout.addWidget(make_row(field_lbl("State", C), self.us_state))

        self.tax_note = QLabel(""); self.tax_note.setStyleSheet(f"color: {C['text3']}; font-size: 10px;")
        layout.addWidget(self.tax_note)
        self._upd_tax(); self.us_state.currentTextChanged.connect(lambda _: self._upd_tax())
        layout.addStretch()

    def _upd_tax(self):
        from data import STATE_TAX_RATES, FEDERAL_RATE
        s = self.us_state.currentText(); sr = STATE_TAX_RATES.get(s, 0)
...
class PanelSchedule(QWidget):
    def __init__(self, settings: dict, C: dict, parent=None):
        ...
        self.deadline_day = QComboBox(); self.deadline_day.addItems(DAYS)
        self.deadline_day.setCurrentText(settings.get("deadline_day", "tuesday").capitalize())
        ...
        self.deadline_time.setTime(QTime(h, m)); self.deadline_time.setDisplayFormat("hh:mm AP")
        ...
        layout.addWidget(sep(C)); layout.addWidget(section_lbl("DEFAULT TARGET HOURS", C))
        note = QLabel("Each workspace and sheet can override independently.")
        note.setStyleSheet(f"color: {C['text3']}; font-size: 10px;"); layout.addWidget(note)

        self.target_hours = QDoubleSpinBox(); self.target_hours.setRange(0, 168)
        self.target_hours.setDecimals(1); self.target_hours.setSuffix(" hrs")
...
class PanelInterface(QWidget):
    def __init__(self, settings: dict, C: dict, parent=None):
        ...
        self.theme = QComboBox(); self.theme.addItems(["dark", "midnight", "slate"])
        self.theme.setCurrentText(settings.get("theme", "dark"))
        layout.addWidget(make_row(field_lbl("Base Theme", C), self.theme))

        layout.addWidget(sep(C)); layout.addWidget(section_lbl("WORKSPACES", C))

        # Startup workspace selector — populated dynamically
        self.startup_ws = QComboBox()
        try:
            from timesheet import list_workspaces
            ws_list = list_workspaces()
        except Exception:
            ws_list = []
        self.startup_ws.addItem("(last used)", "")
        for name in ws_list:
            self.startup_ws.addItem(name, name)
        cur_startup = settings.get("startup_workspace", "")
        idx = self.startup_ws.findData(cur_startup)
        if idx >= 0:
            self.startup_ws.setCurrentIndex(idx)
        layout.addWidget(make_row(field_lbl("Open on startup", C), self.startup_ws))

        layout.addWidget(sep(C)); layout.addWidget(section_lbl("TOASTS", C))

        self.toast_duration = QSpinBox(); self.toast_duration.setRange(1, 30)
        self.toast_duration.setSuffix(" sec"); self.toast_duration.setValue(settings.get("toast_duration", 4))
        ...
        layout.addWidget(sep(C)); layout.addWidget(section_lbl("HOTKEYS", C))
    ...
    def get_values(self) -> dict:
        return {
            ...
            "startup_workspace": self.startup_ws.currentData() or "",
...
class PanelColors(QWidget):
    def __init__(self, settings: dict, C: dict, parent=None):
        super().__init__(parent)
        self.setStyleSheet("background: transparent;"); self._C = C
        outer = QVBoxLayout(self); outer.setContentsMargins(0, 0, 0, 0); outer.setSpacing(0)
        scroll = QScrollArea(); scroll.setWidgetResizable(True)
        scroll.setStyleSheet("QScrollArea { border: none; background: transparent; }")
        inner = QWidget(); inner.setStyleSheet("background: transparent;")
        layout = QVBoxLayout(inner); layout.setContentsMargins(24, 16, 24, 16)
        layout.setSpacing(6); layout.setAlignment(Qt.AlignmentFlag.AlignTop)
        ...
        for key, label in COLOR_LABELS.items():
            ...
            row = ColorRow(key, label, val, C); self._rows[key] = row; layout.addWidget(row)
...
class PanelStorage(QWidget):
    def __init__(self, settings: dict, C: dict, parent=None):
        ...
        path_row = QWidget(); path_row.setStyleSheet("background: transparent;")
        pr_l = QHBoxLayout(path_row); pr_l.setContentsMargins(0, 0, 0, 0); pr_l.setSpacing(8)
        self.data_dir = QLineEdit(settings.get("data_dir", ""))
        self.data_dir.setPlaceholderText("~/.local/share/timesheet"); pr_l.addWidget(self.data_dir, 1)
        browse_btn = QPushButton("Browse"); browse_btn.setFixedWidth(70)
        browse_btn.clicked.connect(self._browse); pr_l.addWidget(browse_btn)
        layout.addWidget(path_row)

        layout.addWidget(sep(C)); layout.addWidget(section_lbl("MAINTENANCE", C))
        ...
        rebuild_btn.clicked.connect(self._rebuild); layout.addWidget(rebuild_btn)
        layout.addStretch()

    def _browse(self):
        d = QFileDialog.getExistingDirectory(self, "Select data folder")
        if d: self.data_dir.setText(d)
    ...
    def _rebuild(self):
        ...
        if not install_sh.exists(): return
        result = subprocess.run(["bash", str(install_sh)], cwd=str(install_sh.parent), capture_output=True, text=True)
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, settings: dict, C: dict, parent=None):
        super().__init__(parent)
        self._settings = dict(settings); self._C = C
        self.setWindowTitle("Settings"); self.setFixedSize(640, 500); self.setModal(True)
        self._build()

    def _build(self):
        C = self._C
        self.setStyleSheet(f"QDialog {{ background: {C['surface']}; border: 1px solid {C['border2']}; }}")
        layout = QHBoxLayout(self); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(0)

        sidebar = QWidget(); sidebar.setFixedWidth(140)
        sidebar.setStyleSheet(f"background: {C['bg']}; border-right: 1px solid {C['border']};")
        sl = QVBoxLayout(sidebar); sl.setContentsMargins(0, 16, 0, 16); sl.setSpacing(2)
        ...
        for i, (name, panel) in enumerate(panels):
            ...
            btn.setStyleSheet(f"""
                ...
                QPushButton:checked {{ color: {C['text']}; background: {C['surface']};
                    border-left: 2px solid {C['accent']}; }}
                QPushButton:hover:!checked {{ color: {C['text']}; background: {C['surface2']}; }}
            """)
            btn.clicked.connect(lambda _, idx=i: self._switch(idx))
            sl.addWidget(btn); self._nav_btns.append(btn)
            self._stack.addWidget(panel); self._panel_objs[name] = panel

        sl.addStretch(); layout.addWidget(sidebar)

        right = QWidget(); right.setStyleSheet(f"background: {C['surface']};")
        rl = QVBoxLayout(right); rl.setContentsMargins(0, 0, 0, 0); rl.setSpacing(0)
        rl.addWidget(self._stack, 1)

        footer = QWidget(); footer.setFixedHeight(50)
        footer.setStyleSheet(f"background: {C['surface']}; border-top: 1px solid {C['border']};")
        fl = QHBoxLayout(footer); fl.setContentsMargins(16, 0, 16, 0); fl.addStretch()
        cancel_btn = QPushButton("Cancel"); cancel_btn.setFixedWidth(80)
        cancel_btn.clicked.connect(self.reject); fl.addWidget(cancel_btn)
        save_btn = QPushButton("Save"); save_btn.setFixedWidth(80)
        save_btn.setStyleSheet(f"""
            QPushButton {{ background: {C['accent']}; border: none; color: white;
                border-radius: 5px; font-size: 11px; }}
            QPushButton:hover {{ background: {C['accent2']}; }}
        """)
        save_btn.clicked.connect(self._save); fl.addWidget(save_btn)
        rl.addWidget(footer); layout.addWidget(right, 1)
